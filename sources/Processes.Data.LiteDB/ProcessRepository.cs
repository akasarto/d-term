...
namespace Processes.Data.LiteDB
{
	public class ProcessRepository : IProcessRepository
	{
		...
		public void Update(ProcessEntity entity)
		{
			using (var database = new LiteDatabase(_connectionString))
			{
				var processes = database.GetCollection<ProcessEntity>(_processesCollection);

				var process = processes.FindOne(n => n.Id == entity.Id);

				if (process != null)
				{
					process.Type = entity.Type;
					process.Name = entity.Name;
					process.OrderIndex = entity.OrderIndex;
					process.PicturePath = entity.PicturePath;
					process.ProcessBasePath = entity.ProcessBasePath;
					process.ProcessExecutableName = entity.ProcessExecutableName;
					process.ProcessStartupArgs = entity.ProcessStartupArgs;
		...
		private void CheckSeed()
		{
			var initialConsoles = new List<ProcessEntity>()
			{
				new ProcessEntity() {
					...
					ProcessBasePath = ProcessBasePath.SystemPathVar,
				...
				new ProcessEntity() {
					...
					ProcessBasePath = ProcessBasePath.ProgramFilesFolder,
				...
				new ProcessEntity() {
					...
					ProcessBasePath = ProcessBasePath.System32Folder,
				...
				new ProcessEntity() {
					Id = Guid.NewGuid(),
					Type = ProcessType.Utility,
					Name = "PuTTY",
					OrderIndex = 5,
					PicturePath = "M4,1C2.89,1 2,1.89 2,3V7C2,8.11 2.89,9 4,9H1V11H13V9H10C11.11,9 12,8.11 12,7V3C12,1.89 11.11,1 10,1H4M4,3H10V7H4V3M3,13V18L3,20H10V18H5V13H3M14,13C12.89,13 12,13.89 12,15V19C12,20.11 12.89,21 14,21H11V23H23V21H20C21.11,21 22,20.11 22,19V15C22,13.89 21.11,13 20,13H14M14,15H20V19H14V15Z",
					ProcessBasePath = ProcessBasePath.App,
					ProcessExecutableName = @"/putty/putty.exe",
					ProcessStartupArgs = string.Empty,
					UTCCreation = DateTime.UtcNow
				}
			};
